...
function getGalaxiesCoordinates(map, emptyLines, emptyColumns) {
  ...
  for (let line = 0; line < map.length; line++) {
    ...
    for (let column = 0; column < map[0].length; column++) {
      if (emptyColumns.has(column)) {
        voidColumns += 999_999;
      }

      if (map[line][column] === "#") {
        galaxiesCoordinates.push([line + voidLines, column + voidColumns]);
      }
    }
  }
...
function main(file) {
  ...
  return getGalaxiesCoordinates(map, emptyLines, emptyColumns)
